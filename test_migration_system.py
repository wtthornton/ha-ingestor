...
class MigrationSystemTester:
        ...
        try:
            ...
            if hasattr(transformer, "transform"):
                ...
                if transformed and transformed.success:
                    ...
                    return {
                        ...
                        "details": {
                            "input": test_data,
                            "errors": (
                                transformed.errors if transformed else "No result"
                            ),
                        },
                    }
            else:
                return {
                    ...
                    "details": {
                        "input": test_data,
                        "available_methods": dir(transformer),
                    },
                }
    ...
    async def test_test_data_generation(self) -> dict[str, Any]:
        ...
        try:
            ...
            config = TestDataConfig(total_events=self.test_data_size, time_span_hours=1)
    ...
    async def run_all_tests(self) -> dict[str, Any]:
        ...
        logger.info(
            "Starting migration system tests",
        ...
        overall_status = (
            "PASSED"
            if success_rate >= 80
            else "PARTIAL_SUCCESS" if success_rate >= 60 else "FAILED"
        )
    ...
    def generate_report(self) -> str:
        ...
        for test_name, result in self.results.items():
            ...
            status_icon = "✅" if result.get("status") == "PASSED" else "❌"
            report_lines.append(
                f"- **{test_name.title()}**: {status_icon} {result.get('status', 'UNKNOWN')}"
            )
...
async def main():
    ...
    tester = MigrationSystemTester(
        dry_run=args.dry_run, test_data_size=args.test_data_size
    )
