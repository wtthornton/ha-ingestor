...
class InfluxDBPoint(BaseModel):
    ...
    def validate_measurement(cls, v: str) -> str:
        ...
        if not measurement.replace("_", "").replace("-", "").isalnum():
            raise ValueError("Measurement name must contain only alphanumeric characters, underscores, and hyphens")
        ...
        for key, value in v.items():
            ...
            if isinstance(value, (int, float, str, bool)):
                ...
                try:
                    ...
                    raise ValueError(f"Field value for '{key}' cannot be converted to supported type")
    ...
    @classmethod
    def from_event(cls, event: Any, measurement: str, tags: Dict[str, str], fields: Dict[str, Any], timestamp: Optional[datetime] = None) -> "InfluxDBPoint":
    ...
    def get_size_estimate(self) -> int:
        ...
        tags_size = sum(len(k) + len(v) + 2 for k, v in self.tags.items())  # +2 for = and ,
        if tags_size > 0:
            tags_size += 1  # +1 for leading comma
        
        fields_size = sum(len(k) + len(str(v)) + 1 for k, v in self.fields.items())  # +1 for =
        if fields_size > 0:
