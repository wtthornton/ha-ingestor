...
class MQTTEvent(Event):
    ...
    def validate_topic(cls, v: str) -> str:
        ...
        if not re.match(r"^homeassistant/[^/]+/[^/]+/state$", v.strip()):
            raise ValueError("Topic must follow pattern: homeassistant/domain/entity_id/state")
    ...
    @classmethod
    def from_mqtt_message(cls, topic: str, payload: str, timestamp: Optional[datetime] = None) -> "MQTTEvent":
        ...
        if len(topic_parts) != 4 or topic_parts[0] != "homeassistant" or topic_parts[3] != "state":
        ...
        if self.attributes:
            ...
            for attr_name in ["friendly_name", "unit_of_measurement", "device_class"]:
                if attr_name in self.attributes:
                    ...
                    if attr_value and len(attr_value) <= 64:  # InfluxDB tag length limit
