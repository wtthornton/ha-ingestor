...
class AttributeFilter(Filter):
    ...
    def __init__(
        ...
        if operator == "regex" and value:
            ...
            try:
                ...
                self.logger.warning(
                    "Invalid regex pattern, using exact match",
                    pattern=value,
                    error=str(e),
                )
                self.operator = "eq"
    ...
    async def should_process(self, event: Event) -> bool:
        ...
        if not event.attributes:
            self.logger.debug(
                "Event has no attributes, filtering out",
        ...
        try:
            ...
            if should_process:
                self.logger.debug(
                    "Attribute matches filter criteria",
                    attribute=self.attribute,
            ...
            self.logger.error(
                "Error comparing attribute values",
                attribute=self.attribute,
                value=attribute_value,
                operator=self.operator,
                error=str(e),
    ...
    def _compare_values(self, attribute_value: Any, filter_value: Any) -> bool:
        ...
        if self.operator == "eq":
            ...
            return self._numeric_compare(
                attribute_value, filter_value, lambda a, b: a > b
            )
    ...
    def update_operator(self, operator: str) -> None:
        ...
        self._validate_operator()
        self.logger.info(
            "Updated filter operator", old_operator=old_operator, new_operator=operator
        )
