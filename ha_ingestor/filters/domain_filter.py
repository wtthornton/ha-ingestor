...
class DomainFilter(Filter):
    ...
    def __init__(self, domains: Union[List[str], str], name: str = None):
        ...
        """
        super().__init__(name or f"domain_filter_{'_'.join(self._normalize_domains(domains))}")
        ...
        self.logger.info("Domain filter initialized", 
                        allowed_domains=self.domains,
                        total_domains=len(self.domains))
    
    def _normalize_domains(self, domains: Union[List[str], str]) -> List[str]:
    ...
    async def should_process(self, event: Event) -> bool:
        ...
        """
        if not event.domain:
            self.logger.debug("Event has no domain, filtering out", 
        ...
        if should_process:
            self.logger.debug("Event domain allowed", 
                            domain=event.domain,
                            entity_id=event.entity_id)
        else:
            self.logger.debug("Event domain filtered out", 
                            domain=event.domain,
                            entity_id=event.entity_id,
                            allowed_domains=self.domains)
    ...
    def add_domain(self, domain: str) -> None:
        ...
        if normalized_domain and normalized_domain not in self._domain_set:
            ...
            self.logger.info("Added domain to filter", 
    ...
    def remove_domain(self, domain: str) -> bool:
        ...
        if normalized_domain in self._domain_set:
            ...
            self.logger.info("Removed domain from filter", 
                           domain=normalized_domain,
                           total_domains=len(self.domains))
            return True
        return False
