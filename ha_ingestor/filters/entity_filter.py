...
class EntityFilter(Filter):
    ...
    def __init__(
        self, patterns: list[str] | str, name: str | None = None, use_regex: bool = True
    ):
        ...
        """
        super().__init__(
            name or f"entity_filter_{'_'.join(self._normalize_patterns(patterns))}"
        )
        ...
        self.logger.info(
            "Entity filter initialized",
            patterns=self.patterns,
            use_regex=use_regex,
            total_patterns=len(self.patterns),
        )

    def _normalize_patterns(self, patterns: list[str] | str) -> list[str]:
    ...
    def _compile_patterns(self) -> list[Pattern]:
        ...
        for pattern in self.patterns:
            try:
                ...
                self.logger.warning(
                    "Invalid regex pattern, skipping", pattern=pattern, error=str(e)
                )
    ...
    async def should_process(self, event: Event) -> bool:
        ...
        self.logger.debug(
            "Entity ID filtered out", entity_id=event.entity_id, patterns=self.patterns
        )
        return False
    ...
    def get_pattern_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pattern_cache_hit_rate": self._cache_hits
