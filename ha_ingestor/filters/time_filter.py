...
class TimeFilter(Filter):
    ...
    def __init__(self, 
        ...
        self.logger.info("Time filter initialized", 
    ...
    def _setup_business_hours(self) -> None:
        ...
        self.logger.info("Business hours configured", 
                        time_ranges=self.time_ranges,
                        days_of_week=self.days_of_week)
    ...
    def _validate_config(self) -> None:
        ...
        for day in self.days_of_week:
            if not isinstance(day, int) or day < 0 or day > 6:
                raise ValueError("Days of week must be integers 0-6 (Monday=0, Sunday=6)")
    ...
    async def should_process(self, event: Event) -> bool:
        ...
        if not event.timestamp:
            self.logger.debug("Event has no timestamp, filtering out", 
        ...
        if self.days_of_week and event_time.weekday() not in self.days_of_week:
            self.logger.debug("Event day not allowed", 
        ...
        if not time_allowed:
            self.logger.debug("Event time not allowed", 
                            event_time=event_time,
                            time_of_day=event_time_of_day,
                            time_ranges=self.time_ranges)
            return False
        
        self.logger.debug("Event time allowed", 
                         event_time=event_time,
                         time_of_day=event_time_of_day)
        return True
    ...
    def _is_time_in_range(self, event_time: time, start_time: time, end_time: time) -> bool:
    ...
    def add_time_range(self, start_time: time, end_time: time) -> None:
        ...
        self.logger.info("Added time range", 
    ...
    def remove_time_range(self, start_time: time, end_time: time) -> bool:
        ...
        try:
            ...
            self.logger.info("Removed time range", 
                           start_time=start_time,
                           end_time=end_time,
                           total_ranges=len(self.time_ranges))
            return True
    ...
    def add_day_of_week(self, day: int) -> None:
        ...
        if day not in self.days_of_week:
            ...
            self.logger.info("Added day of week", 
                           day=day,
    ...
    def remove_day_of_week(self, day: int) -> bool:
        ...
        if day in self.days_of_week:
            ...
            self.logger.info("Removed day of week", 
                           day=day,
                           total_days=len(self.days_of_week))
            return True
        return False
