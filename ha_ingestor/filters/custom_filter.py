...
class CustomFilter(Filter):
    ...
    def __init__(self, 
        ...
        self.logger.info("Custom filter initialized", 
    ...
    async def should_process(self, event: Event) -> bool:
        ...
        try:
            ...
            if not isinstance(result, bool):
                self.logger.warning("Custom filter function returned non-boolean, converting", 
                                  result=result,
                                  filter_func=self.filter_func.__name__)
                result = bool(result)
            
            if result:
                self.logger.debug("Custom filter allowed event", 
                                filter_func=self.filter_func.__name__,
                                event_entity_id=event.entity_id)
            else:
                self.logger.debug("Custom filter blocked event", 
                                filter_func=self.filter_func.__name__,
                                event_entity_id=event.entity_id)
            ...
            self.logger.error("Error in custom filter function", 
                            filter_func=self.filter_func.__name__,
                            error=str(e),
                            event_entity_id=event.entity_id)
    ...
    async def transform(self, event: Event) -> Event:
        ...
        try:
            ...
            if not isinstance(transformed_event, Event):
                self.logger.warning("Custom transform function returned non-Event, using original", 
                                  result_type=type(transformed_event),
                                  transform_func=self.transform_func.__name__)
                return event
            
            self.logger.debug("Event transformed by custom function", 
                            transform_func=self.transform_func.__name__,
                            event_entity_id=event.entity_id)
            return transformed_event
            
        except Exception as e:
            self.logger.error("Error in custom transform function", 
                            transform_func=self.transform_func.__name__,
                            error=str(e),
                            event_entity_id=event.entity_id)
    ...
    def update_filter_function(self, filter_func: Callable) -> None:
        ...
        self.logger.info("Updated filter function", 
                        old_func=old_func,
                        new_func=filter_func.__name__)
    
    def update_transform_function(self, transform_func: Optional[Callable]) -> None:
        ...
        self.logger.info("Updated transform function", 
                        old_func=old_func,
    ...
    def get_filter_config(self) -> Dict[str, Any]:
        ...
        return {
            "filter_func": self.filter_func.__name__,
            "transform_func": self.transform_func.__name__ if self.transform_func else None,
...
def example_priority_filter(event: Event, priority_entities: list = None) -> bool:
