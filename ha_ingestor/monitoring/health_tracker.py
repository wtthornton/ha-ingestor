...
class HealthTracker:
    ...
    def __init__(self):
        ...
        self._on_service_unhealthy: Optional[Callable[[str, ServiceHealth], None]] = None
        self._on_service_recovered: Optional[Callable[[str, ServiceHealth], None]] = None
    
    def register_service(self, service_name: str, health_check: Optional[Callable] = None) -> None:
    ...
    def unregister_service(self, service_name: str) -> None:
        ...
        self.logger.info("Unregistered service from health tracking", service=service_name)
    ...
    def set_service_status(self, service_name: str, status: ServiceStatus, 
        ...
        # Log status change
        self.logger.info("Service status changed", 
        ...
        if previous_status != status:
            self._check_health_state_change(service_name, service, previous_status, status)
    ...
    def record_error(self, service_name: str, error: str, 
        ...
        self.logger.warning("Service error recorded", 
                           service=service_name, error=error, details=details)
    
    def record_warning(self, service_name: str, warning: str, 
                      details: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.logger.warning("Service warning recorded", 
                           service=service_name, warning=warning, details=details)
    
    def set_health_callbacks(self, on_unhealthy: Optional[Callable] = None,
    ...
    async def _check_all_services(self) -> None:
        ...
        for service_name, health_check in self._health_checks.items():
            try:
                ...
                self.logger.error("Error checking service health", 
    ...
    async def _check_service_health(self, service_name: str, health_check: Callable) -> None:
        ...
        try:
            ...
            if result is True or result is None:
                ...
                error_msg = str(result) if result is not False else "Health check failed"
        ...
        for service_name, service in self._services.items():
            if (service.last_heartbeat and 
                ...
                if time_since_heartbeat > self._heartbeat_timeout:
                    self.logger.warning("Service heartbeat timeout", 
                                      service=service_name,
                                      seconds_since_heartbeat=time_since_heartbeat)
                    
                    # Mark service as degraded
                    self.set_service_status(service_name, ServiceStatus.DEGRADED, {
                        "warning": f"Heartbeat timeout ({time_since_heartbeat:.1f}s)"
                    })
    
    def _check_health_state_change(self, service_name: str, service: ServiceHealth,
        ...
        if (previous_status in [ServiceStatus.RUNNING, ServiceStatus.DEGRADED] and
            new_status in [ServiceStatus.ERROR, ServiceStatus.STOPPED]):
            
            if self._on_service_unhealthy:
                try:
                    self._on_service_unhealthy(service_name, service)
                except Exception as e:
                    self.logger.error("Error in unhealthy callback", 
                                    service=service_name, error=str(e))
        
        # Check if service recovered
        elif (previous_status in [ServiceStatus.ERROR, ServiceStatus.DEGRADED] and
            ...
            if self._on_service_recovered:
                try:
                    ...
                    self.logger.error("Error in recovered callback", 
                                    service=service_name, error=str(e))
    
    def _update_service_metrics(self, service_name: str, service: ServiceHealth) -> None:
        ...
        self.metrics_collector.collect_component_metrics(f"{service_name}_service", {
            "status": status_value,
    ...
    def is_healthy(self) -> bool:
        ...
        """
        return self.get_overall_health_status() in [ServiceStatus.RUNNING, ServiceStatus.DEGRADED]
...
_global_tracker: Optional[HealthTracker] = None
