...
class ConnectionMetrics:
    ...
    def update_throughput(self, bytes_transferred: int, duration_seconds: float) -> None:
        """Update throughput calculation."""
        if duration_seconds > 0:
            current_throughput = (bytes_transferred * 8) / duration_seconds  # bits per second
            alpha = 0.1
            if self.throughput_bps == 0.0:
                self.throughput_bps = current_throughput
            else:
                self.throughput_bps = (alpha * current_throughput + (1 - alpha) * self.throughput_bps)
...
class ConnectionMonitor:
    ...
    def __init__(self):
        ...
        self._degraded_threshold = 1000.0  # ms - response time threshold for degraded status
        self._failing_threshold = 5000.0  # ms - response time threshold for failing status
    ...
    async def _check_all_connections(self) -> None:
        ...
        for service_name, health_check in self._health_checks.items():
            try:
                ...
                self.logger.error("Error checking connection health", 
                                service=service_name, error=str(e))
    
    async def _check_connection(self, service_name: str, health_check: Callable) -> None:
        ...
        try:
            ...
            self.logger.warning("Connection health check failed", 
                              service=service_name, error=error_message)
    
    def _update_connection_metrics(self, service_name: str, connection: ConnectionHealth) -> None:
        ...
        self.metrics_collector.record_client_connection_status(service_name, connection.is_healthy())
        
        # Record connection quality metrics
        self.metrics_collector.collect_component_metrics(f"{service_name}_connection", {
    ...
    def record_connection_event(self, service_name: str, event_type: str, 
        ...
        if event_type == "connect":
            ...
            self.logger.info("Connection reestablished", service=service_name, 
                           reconnect_count=connection.metrics.reconnect_count)
            ...
            self.logger.warning("Connection error", service=service_name, details=details)
    ...
    def get_connection_health(self, service_name: str) -> Optional[ConnectionHealth]:
    ...
    def is_healthy(self) -> bool:
        ...
        """
        return self.get_overall_health_status() in [ConnectionStatus.CONNECTED, ConnectionStatus.DEGRADED]
