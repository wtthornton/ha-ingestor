...
class MetricsCollector:
    ...
    def _setup_default_metrics(self) -> None:
        ...
        self.registry.register_gauge(
            "ha_ingestor_uptime_seconds", "Service uptime in seconds"
        )

        # Event processing metrics
        self.registry.register_counter(
            "ha_ingestor_events_processed_total", "Total number of events processed"
        )
        self.registry.register_counter(
            "ha_ingestor_events_mqtt_total", "Total number of MQTT events processed"
        )
        self.registry.register_counter(
        ...
        self.registry.register_histogram(
            "ha_ingestor_connection_latency_seconds", "Connection latency in seconds"
        )
        self.registry.register_histogram(
        ...
        self.registry.register_counter(
            "ha_ingestor_connection_errors_total", "Total number of connection errors"
        )
        self.registry.register_counter(
        ...
        self.registry.register_gauge(
            "ha_ingestor_connection_uptime_seconds", "Connection uptime in seconds"
        )
        ...
        self.registry.register_counter(
            "ha_ingestor_filters_cache_hits_total", "Total number of filter cache hits"
        )
        self.registry.register_counter(
        ...
        self.registry.register_gauge(
            "ha_ingestor_filters_cache_size", "Current size of filter caches"
        )
        self.registry.register_histogram(
        ...
        self.registry.register_counter(
            "ha_ingestor_errors_total", "Total number of errors encountered"
        )
        self.registry.register_counter(
            "ha_ingestor_errors_by_category_total", "Total number of errors by category"
        )
        self.registry.register_counter(
            "ha_ingestor_errors_by_severity_total", "Total number of errors by severity"
        )
        self.registry.register_counter(
        ...
        self.registry.register_counter(
            "ha_ingestor_retry_attempts_total", "Total number of retry attempts"
        )
        ...
        self.registry.register_gauge(
            "ha_ingestor_influxdb_batch_size", "Current InfluxDB batch size"
        )
        ...
        self.logger.info(
            "Default metrics registered", count=len(self.registry.get_metric_names())
        )
    ...
    def record_pipeline_processing_time(self, duration_seconds: float) -> None:
        ...
        self.registry.observe_histogram(
            "ha_ingestor_pipeline_processing_duration_seconds", duration_seconds
        )

    def record_client_connection_status(
        self, client_type: str, connected: bool
    ) -> None:
    ...
    def record_connection_latency(
        self, client_type: str, latency_seconds: float
    ) -> None:
    ...
    def record_connection_response_time(
        self, client_type: str, response_time_seconds: float
    ) -> None:
    ...
    def record_connection_throughput(
        self, client_type: str, throughput_bps: float
    ) -> None:
    ...
    def record_influxdb_write(
        self, points_count: int, success: bool, duration_seconds: float
    ) -> None:
        ...
        if success:
            self.registry.increment_counter(
                "ha_ingestor_influxdb_points_written_total", value=points_count
            )
        else:
            self.registry.increment_counter(
                "ha_ingestor_influxdb_points_failed_total", value=points_count
            )

        # Record write duration
        self.registry.observe_histogram(
            "ha_ingestor_influxdb_write_duration_seconds", duration_seconds
        )
    ...
    def record_influxdb_batch_processed(
        self,
        success: bool,
        ...
        # Record batch age
        self.registry.observe_histogram(
            "ha_ingestor_influxdb_batch_age_seconds", batch_age
        )
        ...
        if original_size > 0:
            ...
            self.registry.set_gauge(
                "ha_ingestor_influxdb_batch_optimization_efficiency", efficiency
            )
        ...
        if compressed_data_size > 0:
            ...
            self.registry.set_gauge(
                "ha_ingestor_influxdb_batch_compression_ratio", compression_ratio
            )
    ...
    def record_influxdb_circuit_breaker_state(self, state: str) -> None:
        ...
        self.registry.set_gauge(
            "ha_ingestor_influxdb_circuit_breaker_state", state_value
        )
    ...
    def record_influxdb_throughput(
        self, points_per_second: float, batches_per_second: float
    ) -> None:
        ...
        """
        self.registry.observe_histogram(
            "ha_ingestor_influxdb_throughput_points_per_second", points_per_second
        )
        self.registry.observe_histogram(
            "ha_ingestor_influxdb_throughput_batches_per_second", batches_per_second
        )
    ...
    def collect_component_metrics(
        self, component_name: str, metrics: dict[str, Any]
    ) -> None:
        ...
        try:
            for metric_name, value in metrics.items():
                if isinstance(value, (int, float)):
                    ...
                    if full_metric_name not in self.registry.get_metric_names():
                        self.registry.register_gauge(
                            full_metric_name, f"{component_name} {metric_name}"
                        )
            ...
            self.logger.error(
                "Failed to collect component metrics",
    ...
    async def timed_operation(
        ...
        try:
            ...
            if metric_name not in self.registry.get_metric_names():
                self.registry.register_histogram(
                    metric_name, f"Duration of {operation_name} operation"
                )
    ...
    def record_error(
        self, category: str, severity: str, recovered: bool = False
    ) -> None:
        ...
        try:
            ...
            self.registry.increment_counter("ha_ingestor_errors_by_category_total")

            # Increment severity-specific counter
            self.registry.increment_counter("ha_ingestor_errors_by_severity_total")
    ...
    def record_circuit_breaker_open(self, component: str) -> None:
        ...
        """
        try:
            self.registry.increment_counter("ha_ingestor_circuit_breaker_opens_total")
        except Exception as e:
    ...
    def record_circuit_breaker_state(self, component: str, state: str) -> None:
        ...
        try:
            ...
            self.registry.set_gauge(
                "ha_ingestor_circuit_breaker_state", float(state_value)
            )
        except Exception as e:
    ...
    def record_retry_attempt(self, component: str, operation: str) -> None:
        ...
        """
        try:
            self.registry.increment_counter("ha_ingestor_retry_attempts_total")
        except Exception as e:
    ...
    def record_filter_metrics(
        ...
        try:
            # Record processing duration
            self.registry.observe_histogram(
                "ha_ingestor_filters_processing_duration_seconds", processing_time
            )

            # Record cache metrics
            if cache_hit:
                self.registry.increment_counter("ha_ingestor_filters_cache_hits_total")
            else:
                self.registry.increment_counter(
                    "ha_ingestor_filters_cache_misses_total"
                )
    ...
    def record_filter_chain_metrics(
        self,
        total_filters: int,
        total_processed: int,
        total_filtered: int,
        ...
        try:
            ...
            self.registry.observe_histogram(
                "ha_ingestor_filter_chain_processing_duration_seconds", processing_time
            )

            # Record filter chain statistics
            self.registry.set_gauge(
                "ha_ingestor_filter_chain_total_filters", float(total_filters)
            )

            # Calculate and record filter rate
            filter_rate = total_filtered / max(total_processed + total_filtered, 1)
            self.registry.set_gauge("ha_ingestor_filter_chain_filter_rate", filter_rate)

        except Exception as e:
            self.logger.error("Failed to record filter chain metrics", error=str(e))

    def update_filter_cache_size(
        self, filter_name: str, filter_type: str, cache_size: int
    ) -> None:
        ...
        """
        try:
            self.registry.set_gauge("ha_ingestor_filters_cache_size", float(cache_size))
        except Exception as e:
