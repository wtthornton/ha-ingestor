...
class Metric:
    ...
    buckets: list[float] | None = None  # For histogram metrics
...
class MetricsRegistry:
    ...
    def register_counter(self, name: str, help_text: str) -> None:
        ...
        self._metrics[name] = Metric(name=name, help_text=help_text, type="counter")
    ...
    def register_gauge(self, name: str, help_text: str) -> None:
        ...
        self._metrics[name] = Metric(name=name, help_text=help_text, type="gauge")
    ...
    def register_histogram(
        ...
        metric = Metric(name=name, help_text=help_text, type="histogram")
        ...
        self.logger.debug(
            "Registered histogram metric", metric_name=name, buckets=buckets
        )
    ...
    def increment_counter(
        ...
        if metric.type != "counter":
            self.logger.error(
                "Metric is not a counter", metric_name=name, metric_type=metric.type
            )
            return
        ...
        self.logger.debug(
            "Incremented counter", metric_name=name, value=value, labels=labels
        )
    ...
    def set_gauge(
        ...
        if metric.type != "gauge":
            self.logger.error(
                "Metric is not a gauge", metric_name=name, metric_type=metric.type
            )
            return
    ...
    def observe_histogram(
        ...
        if metric.type != "histogram":
            self.logger.error(
                "Metric is not a histogram", metric_name=name, metric_type=metric.type
            )
            return
        ...
        self.logger.debug(
            "Observed histogram", metric_name=name, value=value, labels=labels
        )
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        for name, metric in self._metrics.items():
            ...
            if metric.values:
                ...
                latest_timestamp = metric.values[-1].timestamp
