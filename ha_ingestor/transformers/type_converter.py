...
class TypeConverter(Transformer):
    ...
    def __init__(self, name: str, config: dict[str, Any] | None = None):
        ...
        self.type_mappings: dict[str, str] = self.config.get("type_mappings", {})
        self.default_values: dict[str, Any] = self.config.get("default_values", {})
        self.validation_rules: dict[str, dict[str, Any]] = self.config.get(
            "validation_rules", {}
        )
        self.strict_mode: bool = self.config.get("strict_mode", False)
        self.handle_errors: str = self.config.get(
            "handle_errors", "warn"
        )  # warn, skip, fail
                ...
                if field_name in transformed_data:
                    try:
                        ...
                        converted_value = self._convert_value(
                            original_value, target_type, field_name
                        )
                        ...
                        if field_name in self.validation_rules:
                            ...
                            if not validation_result["valid"]:
                                if self.strict_mode:
                                    errors.append(
                                        f"Validation failed for {field_name}: {validation_result['message']}"
                                    )
                                else:
                                    warnings.append(
                                        f"Validation warning for {field_name}: {validation_result['message']}"
                                    )

                    except Exception as e:
                        error_msg = (
                            f"Type conversion failed for field '{field_name}': {str(e)}"
                        )
                        ...
                        if field_name in self.default_values:
                            transformed_data[field_name] = self.default_values[
                                field_name
                            ]
                            self.logger.info(
                    ...
                    if field_name in self.default_values:
                        ...
                        conversions_applied += 1
                        self.logger.debug(
                            "Applied default value for missing field",
                            field=field_name,
                            default_value=self.default_values[field_name],
                        )
            ...
            if not errors:
                ...
                return TransformationResult(
                    ...
                    metadata={
                        ...
                        "validation_rules_applied": len(
                            [
                                f
                                for f in self.type_mappings
                                if f in self.validation_rules
                            ]
                        ),
                    },
    ...
    def _convert_value(self, value: Any, target_type: str, field_name: str) -> Any:
        ...
        if not callable(converter):
            raise ValueError(f"Invalid converter for type {target_type}")
    ...
    def add_type_mapping(self, field_name: str, target_type: str) -> None:
        ...
        self.logger.info(
            "Added type mapping", field=field_name, target_type=target_type
        )

    def add_validation_rule(
        self, field_name: str, rule_type: str, rule_value: Any
    ) -> None:
