...
class CustomTransformer(Transformer):
    ...
    def __init__(self, name: str, config: Optional[Dict[str, Any]] = None):
        ...
        self.transform_function = self.config.get("transform_function")
        self.pre_process_function = self.config.get("pre_process_function")
        self.post_process_function = self.config.get("post_process_function")
        self.error_handler = self.config.get("error_handler")
        self.function_context = self.config.get("function_context", {})
        
        # Validation
        if not self.transform_function and not self.pre_process_function and not self.post_process_function:
        ...
        self.logger.info("CustomTransformer initialized", 
    ...
    def _execute_function(self, func: Callable, data: Dict[str, Any], function_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                if "success" in result:
                    ...
                    if "execution_time_ms" not in result:
                        result["execution_time_ms"] = (time.time() - func_start_time) * 1000
                    return result
                else:
            ...
            if self.error_handler:
                try:
                    error_result = self.error_handler(e, data, function_name, self.function_context)
                    if isinstance(error_result, dict) and "success" in error_result:
                        ...
                        if "execution_time_ms" not in error_result:
                            error_result["execution_time_ms"] = (time.time() - func_start_time) * 1000
                        return error_result
                    else:
                        return {
                            "success": False,
    ...
    def should_apply(self, data: Dict[str, Any]) -> bool:
        ...
        if "custom_condition" in conditions and callable(conditions["custom_condition"]):
            try:
                return conditions["custom_condition"](data, self.function_context)
            except Exception as e:
