...
class ChainMetrics(TypedDict):
    """Type-safe metrics for transformation chains."""

    chain_executions_total: int
    chain_executions_success: int
    chain_executions_failed: int
    total_chain_processing_time_ms: float
...
class TransformationRule(BaseModel):
    ...
    description: str | None = Field(
        None, description="Description of what this rule does"
    )
    ...
    priority: int = Field(
        100, description="Priority for rule execution order (lower = higher priority)"
    )

    # Rule-specific configuration
    config: dict[str, Any] = Field(
        default_factory=dict,
        description="Configuration specific to transformation type",
    )

    # Conditions for when to apply this rule
    conditions: dict[str, Any] | None = Field(
        None, description="Conditions that must be met to apply this rule"
    )
    ...
    def remove_transformer(self, transformer_name: str) -> bool:
        ...
        for i, transformer in enumerate(self.transformers):
            if transformer.name == transformer_name:
                ...
                self.logger.info(
                    "Removed transformer from chain", transformer=transformer_name
                )
                return True
        return False
            ...
            for transformer in self.transformers:
                ...
                try:
                    ...
                    if result.success:
                        ...
                        # Type assertion to help MyPy understand this is a list
                        transformers_applied = metadata["transformers_applied"]
                        if isinstance(transformers_applied, list):
                            transformers_applied.append(
                                {
                        ...
                        if result.warnings:
                            warnings.extend(
                                [f"{transformer.name}: {w}" for w in result.warnings]
                            )
                    else:
                        errors.extend(
                            [f"{transformer.name}: {e}" for e in result.errors]
                        )
                        if result.warnings:
                            warnings.extend(
                                [f"{transformer.name}: {w}" for w in result.warnings]
