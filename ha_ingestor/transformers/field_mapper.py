...
class FieldMapper(Transformer):
    ...
    def __init__(self, name: str, config: Optional[Dict[str, Any]] = None):
        ...
        self.remove_unmapped = self.config.get("remove_unmapped", False)
        self.add_prefix = self.config.get("add_prefix", "")
        self.add_suffix = self.config.get("add_suffix", "")
        self.case_transform = self.config.get("case_transform", "none")  # none, lower, upper, title
        
        # Validation
        if not isinstance(self.field_mappings, dict):
            raise ValueError("field_mappings must be a dictionary")
        
        self.logger.info("FieldMapper initialized", 
        ...
        try:
            ...
            for source_field, target_field in self.field_mappings.items():
                if source_field in data:
                    ...
                    self.logger.debug("Mapped field", 
                                    source=source_field, 
    ...
    def add_field_mapping(self, source_field: str, target_field: str) -> None:
        ...
        self.logger.info("Added field mapping", source=source_field, target=target_field)
