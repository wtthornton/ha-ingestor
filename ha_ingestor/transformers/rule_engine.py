...
class TransformationRuleEngine:
    ...
    def get_rules_by_type(
        self, rule_type: TransformationType
    ) -> list[TransformationRule]:
        """Get all rules of a specific type."""
        return [
            rule
            for rule in self.rules
            if rule.transformation_type == rule_type and rule.enabled
        ]
    ...
    def create_transformer_from_rule(self, rule: TransformationRule) -> Transformer:
        ...
        if rule.transformation_type == TransformationType.FIELD_MAPPING:
            ...
            raise ValueError(
                f"Unsupported transformation type: {rule.transformation_type}"
            )
    ...
    def create_chain_from_rules(
        self, chain_name: str, rule_names: list[str]
    ) -> TransformationChain:
    ...
    def execute_rules(
        self, data: dict[str, Any], rule_names: list[str] | None = None
    ) -> dict[str, Any]:
            ...
            try:
                ...
                if rule.conditions:
                    if not self._evaluate_conditions(rule.conditions, current_data):
                        self.logger.debug(
                            "Rule conditions not met", rule_name=rule.name
                        )
                        continue
                ...
                self.logger.error(
                    "Rule execution failed with exception",
                    rule_name=rule.name,
                    error=str(e),
                )
    ...
    def execute_chain(self, chain_name: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if result.success:
            ...
            self.logger.warning(
                "Chain execution completed with errors",
                chain_name=chain_name,
        ...
        for condition_type, condition_value in conditions.items():
            if condition_type == "required_fields":
                ...
                try:
                    ...
                    self.logger.warning(
                        "Custom condition evaluation failed", error=str(e)
                    )
                    return False
    ...
    def load_rules_from_config(self, config: dict[str, Any]) -> None:
        ...
        for rule_config in rules_config:
            try:
                ...
                self.logger.error(
                    "Failed to load rule from config",
                    rule_config=rule_config,
                    error=str(e),
                )

        self.logger.info(
            "Loaded rules from configuration", rules_count=len(rules_config)
        )
        ...
        if "rules" in config:
            for rule_config in config["rules"]:
                try:
                    ...
                    self.logger.error(
                        "Failed to import rule", rule_config=rule_config, error=str(e)
                    )
        ...
        if "chains" in config:
            for chain_name, chain_config in config["chains"].items():
                if "transformers" in chain_config:
                    self.create_chain_from_rules(
                        chain_name, chain_config["transformers"]
                    )
