"""InfluxDB writer for Home Assistant data storage."""
...
class InfluxDBWriter:
    ...
    async def write_points(self, points: List[InfluxDBPoint]) -> bool:
        ...
        try:
            ...
            self.logger.info("Wrote points to InfluxDB", 
                           total=len(points), 
    ...
    def update_batch_config(self, batch_size: Optional[int] = None, 
                           batch_timeout: Optional[float] = None,
                           compression: Optional[str] = None,
                           compression_level: Optional[int] = None,
                           optimize_batches: Optional[bool] = None) -> None:
        ...
        if compression_level is not None:
            ...
            self.logger.info("Updated compression level", new_level=self._compression_level)
        
        if optimize_batches is not None:
            self._optimize_batches = optimize_batches
            self.logger.info("Updated batch optimization", new_setting=self._optimize_batches)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        # Calculate averages
        avg_batch_size = (self._points_written / self._batches_processed 
                         if self._batches_processed > 0 else 0)
        avg_write_time = (self._total_write_time / self._batches_processed 
                         if self._batches_processed > 0 else 0)
        avg_batch_time = (self._total_batch_time / self._batches_processed 
                         if self._batches_processed > 0 else 0)
    ...
    def get_batch_performance(self) -> Dict[str, Any]:
        ...
        if self._last_write_time and self._last_batch_time:
            total_runtime = (self._last_write_time - self._last_batch_time).total_seconds()
            if total_runtime > 0:
        ...
        efficiency_ratio = (self._total_write_time / self._total_batch_time 
                          if self._total_batch_time > 0 else 0.0)
        ...
        return {
            ...
            "avg_write_time": round(self._total_write_time / self._batches_processed, 3),
            "avg_batch_time": round(self._total_batch_time / self._batches_processed, 3),
    ...
    def get_circuit_breaker_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "consecutive_failures": self._consecutive_failures,
            "last_failure_time": (self._circuit_breaker_last_failure_time.isoformat() 
                                 if self._circuit_breaker_last_failure_time else None),
            "last_successful_write": (self._last_successful_write.isoformat() 
    ...
    def update_throughput_metrics(self) -> None:
        ...
        try:
            ...
            if self._last_write_time and self._last_batch_time:
                runtime = (self._last_write_time - self._last_batch_time).total_seconds()
                if runtime > 0:
                    points_per_second = self._points_written / runtime
                    batches_per_second = self._batches_processed / runtime
                    
                    # Record throughput metrics
                    metrics_collector.record_influxdb_throughput(points_per_second, batches_per_second)
                    
                    # Update circuit breaker state metric
                    metrics_collector.record_influxdb_circuit_breaker_state(self._circuit_breaker_state)
    ...
    def detect_workload_type(self, points: List[InfluxDBPoint]) -> Dict[str, Any]:
        ...
        return {
            ...
            "characteristics": {
                ...
                "data_density": total_points / unique_measurements if unique_measurements > 0 else 0
            }
        }
    
    def optimize_for_workload(self, points: List[InfluxDBPoint], workload_info: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        self.logger.debug("Applying workload-specific optimization", 
    ...
    def _optimize_high_cardinality(self, points: List[InfluxDBPoint], characteristics: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        for point in points:
            ...
            for key, value in point.tags.items():
                ...
                elif len(set(p.tags.get(key, '') for p in points if p.measurement == point.measurement)) > 1:
                    essential_tags[key] = value
        ...
        self.logger.debug("Optimized high cardinality workload", 
    ...
    def _optimize_wide_metrics(self, points: List[InfluxDBPoint], characteristics: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        for point in points:
            ...
            for field_name, field_value in point.fields.items():
                if field_name not in grouped_points[group_key]['fields'] or \
                   point.timestamp > grouped_points[group_key]['timestamp']:
        ...
        self.logger.debug("Optimized wide metrics workload", 
                         original_points=len(points),
                         optimized_points=len(optimized_points),
                         avg_fields_before=characteristics.get('avg_field_count', 0),
                         avg_fields_after=sum(len(p.fields) for p in optimized_points) / len(optimized_points) if optimized_points else 0)
        
        return optimized_points
    
    def _optimize_simple_metrics(self, points: List[InfluxDBPoint], characteristics: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        for point in points:
            ...
            key = (point.measurement, point.timestamp, tuple(sorted(point.tags.items())))
            
            if key not in seen:
                seen.add(key)
                optimized_points.append(point)
        
        self.logger.debug("Optimized simple metrics workload", 
                         original_points=len(points),
                         optimized_points=len(optimized_points),
    ...
    def _optimize_multi_source(self, points: List[InfluxDBPoint], characteristics: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        self.logger.debug("Optimized multi-source workload", 
                         measurements=len(measurement_groups),
                         total_points=len(optimized_points))
        
        return optimized_points
    
    def _optimize_burst_workload(self, points: List[InfluxDBPoint], characteristics: Dict[str, Any]) -> List[InfluxDBPoint]:
        ...
        self.logger.debug("Optimized burst workload", 
                         original_points=len(points),
                         optimized_points=len(optimized_points),
    ...
    def get_workload_optimization_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_effectiveness": {
                ...
                "avg_compression_improvement": getattr(self, '_avg_compression_improvement', 0.0),
                "total_optimization_time": getattr(self, '_total_optimization_time', 0.0)
            }
        }
        ...
        try:
            ...
            for point in points_to_write:
                try:
                    ...
                    self.logger.error("Error converting point to line protocol", 
            ...
            if success and self._compression != "none":
                ...
                if self._compression == "gzip":
                    compressed_data_size = int(original_data_size * 0.7)  # ~30% compression
                elif self._compression == "deflate":
                    compressed_data_size = int(original_data_size * 0.75)  # ~25% compression
            ...
            if success:
                ...
                self.logger.debug("Successfully wrote batch to InfluxDB", 
                                points=len(lines),
                                batch_age=batch_age,
                                duration=duration)
            else:
    ...
    async def _write_lines(self, lines: List[str]) -> bool:
        ...
        for attempt in range(self._max_retries + 1):
            ...
            try:
                ...
                # Send write request
                async with self.session.post(url, params=params, data=compressed_data, headers=headers) as response:
                    if response.status == 204:  # No content = success
                        ...
                        self.logger.warning("InfluxDB rate limited, retrying", 
                                          attempt=attempt + 1,
                                          max_retries=self._max_retries)
                        ...
                        self.logger.error("InfluxDB write failed", 
                                        status=response.status,
                                        attempt=attempt + 1,
                                        error=error_text)
                ...
                self.logger.warning("InfluxDB write timeout, retrying", 
                                  attempt=attempt + 1,
                                  max_retries=self._max_retries)
                if attempt < self._max_retries:
                    delay = self._calculate_retry_delay(attempt)
                ...
                self.logger.error("Error writing to InfluxDB", 
                                attempt=attempt + 1,
                                max_retries=self._max_retries,
                                error=str(e))
                if attempt < self._max_retries:
    ...
    def _calculate_retry_delay(self, attempt: int) -> float:
        ...
        self.logger.debug("Calculated retry delay", 
                         attempt=attempt + 1,
    ...
    def _is_circuit_breaker_open(self) -> bool:
        ...
        if self._circuit_breaker_state == "OPEN":
            ...
            if (self._circuit_breaker_last_failure_time and 
                (datetime.now() - self._circuit_breaker_last_failure_time).total_seconds() > self._circuit_breaker_timeout):
    ...
    def _optimize_batch(self, points: List[InfluxDBPoint]) -> List[InfluxDBPoint]:
        ...
        for point in sorted_points:
            ...
            if point_key not in seen_measurements:
                ...
                existing_point = next(p for p in optimized_points if 
                                   f"{p.measurement}:{p.tags.get('entity_id', '')}:{p.timestamp}" == point_key)
        ...
        if removed_count > 0:
            self.logger.debug("Optimized batch", 
    ...
    def _compress_data(self, data: str) -> tuple[bytes, str]:
        ...
        try:
            if self._compression == "gzip":
                compressed = gzip.compress(data.encode('utf-8'), compresslevel=self._compression_level)
                return compressed, "gzip"
            elif self._compression == "deflate":
                compressed = zlib.compress(data.encode('utf-8'), level=self._compression_level)
                return compressed, "deflate"
            else:
                self.logger.warning("Unknown compression algorithm, using none", 
                                  algorithm=self._compression)
                return data.encode('utf-8'), "identity"
                
        except Exception as e:
            self.logger.error("Compression failed, using uncompressed data", 
                            algorithm=self._compression, error=str(e))
            return data.encode('utf-8'), "identity"
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if any(bucket['name'] == self.config.influxdb_bucket for bucket in buckets.get('buckets', [])):
                        self.logger.info("InfluxDB bucket already exists", 
                                       bucket=self.config.influxdb_bucket)
                        return True
                ...
                async with self.session.post(create_url, json=create_data) as create_response:
                    if create_response.status == 201:
                        self.logger.info("Successfully created InfluxDB bucket", 
                                       bucket=self.config.influxdb_bucket)
                        return True
                    else:
                        error_text = await create_response.text()
                        self.logger.error("Failed to create InfluxDB bucket", 
