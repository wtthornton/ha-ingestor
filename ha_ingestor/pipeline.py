...
class PipelineStats:
    ...
    def update_processing_time(self, processing_time: float) -> None:
        ...
        if self.events_processed == 0:
            ...
            alpha = 0.1
            self.processing_time_avg = (alpha * processing_time + 
                                      (1 - alpha) * self.processing_time_avg)
...
class EventProcessor:
    ...
    def __init__(self, config=None):
        ...
        self._max_processing_time = getattr(config, 'pipeline_max_processing_time', 1.0)  # seconds
    ...
    async def process_event(self, event: Union[MQTTEvent, WebSocketEvent]) -> bool:
    ...
    async def process_events(self, events: List[Union[MQTTEvent, WebSocketEvent]]) -> bool:
        ...
        try:
            ...
            self.logger.debug("Queued events for processing", 
                            total=len(events), 
    ...
    async def _process_batch(self, events: List[Union[MQTTEvent, WebSocketEvent]]) -> None:
        ...
        try:
            ...
            if processing_time > self._max_processing_time:
                self.logger.warning("Batch processing took longer than expected", 
    ...
    async def _process_single_event(self, event: Union[MQTTEvent, WebSocketEvent]) -> None:
        """Process a single event through the pipeline.
        ...
        try:
            ...
            if self._is_duplicate(event):
                ...
                self.logger.debug("Event deduplicated", event_id=self._get_event_id(event))
                return
            
            # Apply filters
            if not self._should_process_event(event):
                self.logger.debug("Event filtered out", event_id=self._get_event_id(event))
                return
    ...
    def _is_duplicate(self, event: Union[MQTTEvent, WebSocketEvent]) -> bool:
    ...
    def _should_process_event(self, event: Union[MQTTEvent, WebSocketEvent]) -> bool:
        ...
        if event_type in self._filters:
            try:
                return self._filters[event_type](event)
            except Exception as e:
                self.logger.error("Error applying filter", 
                                event_type=event_type, error=str(e))
                return True  # Default to processing if filter fails
        
        return True
    
    async def _transform_event(self, event: Union[MQTTEvent, WebSocketEvent]) -> Optional[Any]:
        ...
        if event_type in self._transformers:
            try:
                ...
                self.logger.error("Error applying transformer", 
                                event_type=event_type, error=str(e))
        
        return None
    
    async def _store_event(self, event: Union[MQTTEvent, WebSocketEvent]) -> bool:
        ...
        for handler in self._storage_handlers:
            try:
                ...
                self.logger.error("Error in storage handler", 
    ...
    def _get_event_id(self, event: Union[MQTTEvent, WebSocketEvent]) -> str:
    ...
    def _get_event_type(self, event: Union[MQTTEvent, WebSocketEvent]) -> str:
    ...
    def _add_to_dedup_cache(self, event: Union[MQTTEvent, WebSocketEvent]) -> None:
    ...
    def _setup_default_components(self) -> None:
        ...
        def accept_all(event: Union[MQTTEvent, WebSocketEvent]) -> bool:
            return True
