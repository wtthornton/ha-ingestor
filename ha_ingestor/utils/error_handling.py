...
class ErrorRecoveryStrategy:
    ...
    async def attempt_recovery(
        ...
        if not error_info.should_retry():
            self.logger.warning(
                "Error not recoverable, skipping recovery",
        ...
        if self._should_skip_recovery(recovery_key):
            self.logger.info(
                "Skipping recovery due to recent attempts", recovery_key=recovery_key
            )
            return False
        ...
        try:
            if recovery_func:
                if asyncio.iscoroutinefunction(recovery_func):
                    result = await recovery_func(
                        *recovery_args, **recovery_kwargs or {}
                    )
                else:
                ...
                if result:
                    self.logger.info(
                        "Error recovery successful", recovery_key=recovery_key
                    )
                    self._record_successful_recovery(recovery_key)
                    return True
                else:
                    self.logger.warning(
                        "Error recovery failed", recovery_key=recovery_key
                    )
            ...
            self.logger.error(
                "Error during recovery attempt", recovery_key=recovery_key, error=str(e)
            )
            self._record_failed_recovery(recovery_key)
            return False
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    error_info, recovery_success = (
                        await error_handler.handle_error_with_recovery(
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # Handle error (synchronous version)
