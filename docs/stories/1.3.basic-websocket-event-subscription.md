...
- [ ] Task 1: Implement WebSocket event subscription (AC: 1, 4)
  - [ ] Create `subscribe_events` WebSocket message handler
  - [ ] Implement subscription to `state_changed` event type
  - [ ] Add subscription confirmation handling
  - [ ] Implement subscription re-establishment after reconnection
  - [ ] Add subscription status tracking and reporting

- [ ] Task 2: Implement event message processing (AC: 2, 5)
  - [ ] Create event message handler for incoming WebSocket messages
  - [ ] Implement basic event validation (required fields check)
  - [ ] Add event parsing for Home Assistant event structure
  - [ ] Implement event logging with structured format
  - [ ] Add event data extraction (entity_id, state, timestamp, attributes)

- [ ] Task 3: Implement automatic reconnection with subscription (AC: 3, 4)
  - [ ] Extend reconnection logic to re-establish event subscriptions
  - [ ] Implement subscription retry mechanism
  - [ ] Add connection health monitoring during subscription
  - [ ] Implement graceful handling of subscription failures
  - [ ] Add reconnection status logging with subscription details

- [ ] Task 4: Implement event capture rate monitoring (AC: 7)
  - [ ] Create event counter and rate calculation system
  - [ ] Implement time-based event rate tracking (events per minute/hour)
  - [ ] Add event capture statistics to health check endpoint
  - [ ] Implement event rate logging and alerting
  - [ ] Add total event count tracking since service start

- [ ] Task 5: Enhance health check with subscription status (AC: 6)
  - [ ] Extend health check to include subscription status
  - [ ] Add event capture rate to health response
  - [ ] Implement subscription health monitoring
  - [ ] Add last event received timestamp to health status
  - [ ] Include connection and subscription status in health response

- [ ] Task 6: Implement comprehensive event validation (AC: 5)
  - [ ] Create event schema validation for Home Assistant events
  - [ ] Implement required field validation (entity_id, state, timestamp)
  - [ ] Add event type validation (state_changed events only)
  - [ ] Implement malformed event handling and logging
  - [ ] Add event validation error reporting

- [ ] Task 7: Create event processing pipeline foundation (AC: 2)
  - [ ] Create event processor class for handling incoming events
  - [ ] Implement event normalization and formatting
  - [ ] Add event queuing system for processing pipeline
  - [ ] Create event forwarding mechanism to enrichment service
  - [ ] Implement event processing error handling

- [ ] Task 8: Create comprehensive tests (AC: All)
  - [ ] Create `test_event_subscription.py` for subscription testing
  - [ ] Create `test_event_processing.py` for event handling testing
  - [ ] Create `test_reconnection_with_subscription.py` for reconnection testing
  - [ ] Add mock Home Assistant WebSocket server with event simulation
  - [ ] Add integration tests for complete event flow
  - [ ] Add performance tests for event capture rate
