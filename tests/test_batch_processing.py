...
class TestBatchProcessingPerformance:
    ...
    async def test_batch_throughput_metrics(self, writer):
        ...
        mock_collector = Mock()
        with patch(
            "ha_ingestor.influxdb.writer.get_metrics_collector",
...
class TestBatchProcessingReliability:
    ...
    async def test_circuit_breaker_behavior(self, writer):
        ...
        # Wait for timeout
        writer._circuit_breaker_last_failure_time = datetime.now() - timedelta(
            seconds=70
        )
    ...
    async def test_retry_delay_calculation(self, writer):
        ...
        max_delay = writer.config.influxdb_retry_delay * (
            writer.config.influxdb_retry_backoff**4
        )
...
class TestBatchProcessingIntegration:
    ...
    async def test_configuration_integration(self, writer):
        ...
        writer.update_batch_config(batch_size=new_batch_size, batch_timeout=new_timeout)
...
class TestBatchProcessingEdgeCases:
    ...
    async def test_concurrent_batch_operations(self, writer, sample_points):
        ...
        tasks = [write_points(i * 50, 50) for i in range(4)]
