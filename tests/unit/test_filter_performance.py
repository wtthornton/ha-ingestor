...
from ha_ingestor.filters import (
    DomainFilter, EntityFilter, AttributeFilter, FilterChain
)
from ha_ingestor.filters.performance import (
    FilterProfile, FilterChainProfile, FilterProfiler, get_filter_profiler
)
...
class TestFilterProfiler:
    ...
    def test_record_filter_execution(self):
        ...
        profiler.record_filter_execution("test_chain", "filter1", "domain", 15.0, True, False)
        ...
        profile = profiler.chain_profiles["test_chain"].filter_profiles["filter1_domain"]
        assert profile.total_calls == 1
    ...
    def test_identify_bottlenecks(self):
        ...
        profiler.record_filter_execution("test_chain", "slow_filter", "domain", 50.0, False, False)
        profiler.record_filter_execution("test_chain", "slow_filter", "domain", 60.0, False, False)
        
        profiler.record_filter_execution("test_chain", "efficient_filter", "entity", 5.0, False, False)
        profiler.record_filter_execution("test_chain", "efficient_filter", "entity", 5.0, False, False)
...
class TestFilterProfilerIntegration:
    ...
    async def test_profiler_with_real_filters(self):
        ...
        # Create test events
        events = [
            MQTTEvent.from_mqtt_message("homeassistant/light/living_room/state", "on", time.time()),
            MQTTEvent.from_mqtt_message("homeassistant/switch/tv/state", "off", time.time()),
            MQTTEvent.from_mqtt_message("homeassistant/sensor/temp/state", "23.5", time.time())
        ]
        ...
        for i, event in enumerate(events):
            ...
            profiler.record_filter_execution(
                "integration_test_chain", "domain_filter", "domain",
                processing_time, False, False  # Not cached, not filtered
            )
            ...
            if event.domain in ["light", "switch"]:
                ...
                profiler.record_filter_execution(
                    "integration_test_chain", "entity_filter", "entity",
                    processing_time + 1, False, entity_filtered
                )
                
                # Record chain execution
                total_time = processing_time * 2 + 1
                event_filtered = entity_filtered
                profiler.record_chain_execution("integration_test_chain", total_time, event_filtered)
            else:
                # Event filtered by domain filter
                profiler.record_chain_execution("integration_test_chain", processing_time, True)
...
class TestFilterPerformanceOptimizations:
    ...
    async def test_regex_pattern_optimization(self):
        ...
        event = MQTTEvent.from_mqtt_message("homeassistant/light/living_room/state", "on", time.time())
...
class TestMetricsIntegration:
    ...
    async def test_filter_metrics_collection(self):
        ...
        events = [
            MQTTEvent.from_mqtt_message("homeassistant/light/living_room/state", "on", time.time()),
            MQTTEvent.from_mqtt_message("homeassistant/light/kitchen/state", "off", time.time())
        ]
