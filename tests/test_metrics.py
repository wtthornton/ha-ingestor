...
from ha_ingestor.metrics import MetricsCollector, MetricsRegistry, create_metrics_collector
...
class TestMetricsCollector:
    ...
    def test_record_event_processed(self):
        ...
        total_metric = collector.registry.get_metric("ha_ingestor_events_processed_total")
    ...
    def test_record_pipeline_processing_time(self):
        ...
        metric = collector.registry.get_metric("ha_ingestor_pipeline_processing_duration_seconds")
        assert len(metric.values) == 2
        assert metric.values[-1].value == 1.2
    ...
    def test_record_influxdb_write(self):
        ...
        written_metric = collector.registry.get_metric("ha_ingestor_influxdb_points_written_total")
        assert written_metric.values[-1].value == 10.0
        
        duration_metric = collector.registry.get_metric("ha_ingestor_influxdb_write_duration_seconds")
        assert duration_metric.values[-1].value == 0.5
        
        # Record failed write
        collector.record_influxdb_write(5, False, 0.1)
        
        failed_metric = collector.registry.get_metric("ha_ingestor_influxdb_points_failed_total")
    ...
    def test_record_error(self):
        ...
        connection_errors = collector.registry.get_metric("ha_ingestor_connection_errors_total")
    ...
    def test_collect_component_metrics(self):
        ...
        cpu_metric = collector.registry.get_metric("ha_ingestor_test_component_cpu_usage")
        assert cpu_metric is not None
        assert cpu_metric.values[-1].value == 75.5
        
        memory_metric = collector.registry.get_metric("ha_ingestor_test_component_memory_usage")
