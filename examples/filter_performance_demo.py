...
from ha_ingestor.filters import (
    DomainFilter, EntityFilter, AttributeFilter, TimeFilter, CustomFilter, FilterChain
)
...
def create_sample_events(count: int = 1000) -> List[Event]:
    ...
    entity_ids = [
        ...
        "switch.tv", "switch.coffee_maker", "switch.lamp",
        "sensor.temperature", "sensor.humidity", "sensor.motion",
        "binary_sensor.door", "binary_sensor.window", "binary_sensor.motion",
        "climate.thermostat", "climate.ac", "climate.heater",
        "cover.garage_door", "cover.window", "cover.blind"
    ]
...
async def demonstrate_filter_caching():
    ...
    entity_filter = EntityFilter(
        patterns=["light.*", "switch.*", "sensor.*"],
        name="demo_entity_filter"
    )
    ...
    print(f"  Cache misses: {entity_filter._total_processed - entity_filter._cache_hits}")
...
async def demonstrate_filter_chain_profiling():
    ...
    chain.add_filter(AttributeFilter(
        {"state": ["on", "off"]}, name="attribute_filter"
    ))
    chain.add_filter(TimeFilter(
        start_time="09:00", end_time="18:00", name="time_filter"
    ))
...
async def demonstrate_metrics_collection():
    ...
    filter_chain.add_filter(DomainFilter(["light", "switch"], name="demo_domain_filter"))
    ...
    print(f"  - Filter processing duration histograms")
    print(f"  - Filter cache hit/miss counters")
    print(f"  - Filter chain performance metrics")
    print(f"  - Cache size gauges")
    print(f"  - Event processing counters")
