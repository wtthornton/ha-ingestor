...
from ha_ingestor.transformers.base import (
    TransformationType, TransformationRule, TransformationChain
)
...
def demo_custom_transformer():
    ...
    custom_transformer = CustomTransformer("automation_enhancer", custom_transformer_config)
...
def demo_transformation_chain():
    ...
    field_mapper = FieldMapper("field_renamer", {
        "field_mappings": {
            "entity_id": "device_id",
            "state": "current_state",
            "attributes.friendly_name": "device_name"
    ...
    type_converter = TypeConverter("type_converter", {
        "type_mappings": {
    ...
    def custom_transform(data, context, logger, transformer_name):
        ...
        if data.get("domain") == "switch":
            data["switch_metadata"] = {
                ...
                "category": "security" if "garage" in data.get("ha_device_name", "") else "general"
            }
        return data
    
    custom_transformer = CustomTransformer("business_logic", {
    ...
    for name, transformer_metrics in metrics['transformer_metrics'].items():
        print(f"  {name}: {transformer_metrics['transformations_total']} transformations")
...
def demo_rule_engine():
                    ...
                    **data,
                    "light_metrics": {
                        ...
                        "brightness_percentage": (
                            ...
                            if data.get("ha_brightness_level") is not None else 0
                        )
                    }
                }
            }
    ...
    chain = engine.create_chain_from_rules("light_processor", [
        "light_field_mapping",
        "numeric_type_conversion",
        "light_business_logic"
    ])
