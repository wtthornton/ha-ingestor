...
async def demonstrate_domain_filter():
    ...
    for event in events:
        ...
        print(f"{status}: {event.domain}.{event.entity_id} - {event.attributes.get('state', 'N/A')}")
...
async def demonstrate_filter_chain():
    ...
    for event in events:
        ...
        if result:
            print(f"ALLOWED: {event.domain}.{event.entity_id} - {event.attributes.get('state', 'N/A')}")
        else:
            print(f"BLOCKED: {event.domain}.{event.entity_id} - {event.attributes.get('state', 'N/A')}")
